...
- **Automation** — 31 workflow types including 16 AI bot workers with full Bot Command Center in Settings. Core workflows: auto_dispute, client_onboarding, score_monitoring, follow_up, letter_generation, compliance_check, bureau_auto_pull, metro2_furnishing, full_pipeline, ai_credit_worker, tradeline_optimization, stale_dispute_check, payment_reminder, collection_response, client_graduation. AI Bots (16): bot_system_health, bot_banking_sync, bot_document_worker, bot_legal_compliance, bot_data_furnisher, bot_lender_outreach, bot_owner_briefing, bot_security_monitor, bot_accounting, bot_client_comms, bot_coder, bot_trust_law, bot_developer, bot_trainer, bot_credit_specialist, bot_maintenance. Bot Command Center: per-bot toggle, schedule (hourly/daily/weekly/monthly), manual run, action pipeline view, last run results, category filtering (Operations/Financial/Legal/Credit/Development). 5-minute scheduler, admin-only reseed endpoint.
