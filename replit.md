...
- **Automation** — 25 workflow types including 10 AI bot workers. Core: auto_dispute, client_onboarding, score_monitoring, follow_up, letter_generation, compliance_check, bureau_auto_pull, metro2_furnishing, full_pipeline, ai_credit_worker, tradeline_optimization, stale_dispute_check, payment_reminder, collection_response, client_graduation. AI Bots: bot_system_health (hourly), bot_banking_sync (hourly), bot_document_worker (daily), bot_legal_compliance (daily), bot_data_furnisher (monthly), bot_lender_outreach (daily), bot_owner_briefing (daily), bot_security_monitor (hourly), bot_accounting (daily), bot_client_comms (daily). 5-minute scheduler, seed rules, admin-only reseed endpoint.
